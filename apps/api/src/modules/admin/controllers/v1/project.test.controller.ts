...
import { BulkCreateLeoProjectFormsRequestDto, CreateBulkMatchingRoundsDto } from '@upms/shared';
...
export class ProjectTestController {
  ...
  async getProjectPartTo(@Param('projectId') projectId: string, @Query('part') part: UserPartEnum) {
    return this.projectService.checkIfToLeftInProject(projectId, part);
  }

  @ApiOperation({
    summary: '[테스트용] Leo 지부 프로젝트, 폼, 질문 일괄 생성',
    description: '여러 Leo 지부 프로젝트와 각 프로젝트에 해당하는 폼 및 질문들을 일괄 생성합니다.',
  })
  @Post('init/bulk')
  async bulkCreateSampleProjects(@Body() body: BulkCreateLeoProjectFormsRequestDto) {
    const { projects, forms, questions } = body;
    console.log(projects.length, forms.length, questions.length);

    if (projects.length !== forms.length || projects.length !== questions.length) {
      throw new BadRequestException('프로젝트, 폼, 질문 배열의 길이는 모두 동일해야 합니다.');
    }

    const createdProjects = await this.projectService.createBulkProjectWithCreatedEntity(projects);
    this.logger.log(`Created ${createdProjects.length} projects.`);

    await Promise.all(
      createdProjects.map(async (prj, idx) => {
        this.logger.log(`Creating form and questions for project ID: ${prj.id}`);

        this.logger.log(`Created project ID: ${prj.id}`);

        const formForPrj = await this.formService.createForm(prj.id, forms[idx]);
        this.logger.log(`Created form ID: ${formForPrj.id} for project ID: ${prj.id}`);

        await this.formService.addFormQuestions(formForPrj.id, questions[idx].questions);
        this.logger.log(`Added questions to form ID: ${formForPrj.id} for project ID: ${prj.id}`);
      }),
    );
  ...
  async createBulkMatchingRounds(@Body() body: CreateBulkMatchingRoundsDto) {
    for (const round of body.matchingRounds) {
      const createdRound = await this.matchingRoundService.createMatchingRound(round);
